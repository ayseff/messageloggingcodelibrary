...
#pragma warning restore 1591
        ...
        private static string _eventSourceInitializer = WindowsEventLog.GetStringValueFromConfigFile("EventSourceInitializer", string.Empty);
        ...
        private void InitEventLog(string logName, string machineName, string eventSource)
        {
            if (logName == "Unknown" || String.IsNullOrEmpty(logName))
                _logName = WindowsEventLog.GetStringValueFromConfigFile("WindowsEventLog", "Application");
            else
                _logName = logName;

            if (String.IsNullOrEmpty(machineName))
                _machineName = WindowsEventLog.GetStringValueFromConfigFile("WindowsEventLogMachineName", ".");
            else
                _machineName = machineName;

            if (String.IsNullOrEmpty(eventSource))
                _eventSource = WindowsEventLog.GetStringValueFromConfigFile("WindowsEventLogEventSource", "PFApps");
            else
